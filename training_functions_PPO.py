...
def ppo_training_step(states_batch, saccade_generator, old_logprobs, returns, advantages, clipping_val, alpha_critic_loss, alpha_entropy_loss):
    with tf.GradientTape() as actor_tape, tf.GradientTape() as critic_tape:
        ...
        action_probs = tf.squeeze(saccade_generator.actor_network(states_batch))
        ...
        new_values = saccade_generator.critic_network(states_batch)
        ...
        actor_grad = actor_tape.gradient(ppo_loss, saccade_generator.actor_network.trainable_variables)
        critic_grad = critic_tape.gradient(ppo_loss, saccade_generator.critic_network.trainable_variables)
        saccade_generator.optimizer.apply_gradients(zip(actor_grad, saccade_generator.actor_network.trainable_variables))
        saccade_generator.optimizer.apply_gradients(zip(critic_grad, saccade_generator.critic_network.trainable_variables))
...
def train_GSNs(visual_system, saccade_generator, readout_layer, timesteps, init_fix_pos,
    ...
    save_gif_cond = 500  # save gifs every X batches
    ...
    ppo_loss, actor_loss, critic_loss, entropy_loss, KL_divergence, RL_mean_reward = 0., 0., 0., 0., 0., 0.  # need to be defined for tensorboard logs prior to the first ppo update
    ...
    old_log_probs = np.zeros((ppo_buffer_size * timesteps, 1))
            ...
            with tf.GradientTape() as vis_sys_tape:
                ...
                for batch_img in range(img_batch_size):
                    ...
                    for t in range(timesteps):
                        ...
                        action, old_log_prob, observed_log_probs, entropy = saccade_generator.get_action_log_prob_and_entropy(this_state)  # Query actor for the next action
                        ...
                        old_log_probs[states_counter] = old_log_prob
                ...
                if n % 10 == 0:
                    ...
                    with summary_writer.as_default():
                        ...
                        tf.summary.scalar('3e_KL_divergence', tf.reduce_mean(KL_divergence), step=sac_step)
                        tf.summary.scalar('4a_mean_reward', RL_mean_reward, step=sac_step)
                        tf.summary.scalar('4b_mean_predicted_value', tf.reduce_mean(values), step=sac_step)
