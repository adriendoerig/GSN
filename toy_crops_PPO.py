...
img_batch_size = 32  # visual system trained every img_batch_size images, i.e., on img_batch_size*timesteps samples
n_ppo_epochs = 5  # epochs going through ppo_buffer at each ppo update
...
n_hidden_units = 64  # only for 'fully_connected at the moment.
...
clipping_val = 0.1  # for ppo loss
alpha_critic_loss = 0.5  # weight of critic in total ppo loss
alpha_entropy_loss = 1e-2  # weight of entropy in total ppo loss
...
saccade_actor, saccade_actor_ckpt_dict = get_saccade_actor_model(state_shape, n_hidden_units, RL_optimizer, n_actions, sac_net+'_actor', RUN_N)
saccade_critic, saccade_critic_ckpt_dict = get_saccade_critic_model(state_shape, n_hidden_units, RL_optimizer, sac_net+'_critic', RUN_N)
